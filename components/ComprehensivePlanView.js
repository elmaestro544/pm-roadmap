...
const MarkdownRenderer = ({ content }) => {
    ...
    lines.forEach((line, index) => {
        ...
        if (trimmed.startsWith('|')) { inTable = true; tableBuffer.push(trimmed); return; } else if (inTable) { inTable = false; elements.push(renderTable(tableBuffer, `table-${index}`)); tableBuffer = []; }
        if (trimmed.startsWith('### ')) elements.push(React.createElement('h4', { key: index, className: 'text-lg font-bold text-brand-purple-light mt-6 mb-2 print:text-black' }, trimmed.replace('### ', '')));
        else if (trimmed.startsWith('## ')) elements.push(React.createElement('h3', { key: index, className: 'text-xl font-bold text-white mt-8 mb-3 print:text-black' }, trimmed.replace('## ', '')));
        else if (trimmed.startsWith('# ')) elements.push(React.createElement('h2', { key: index, className: 'text-2xl font-bold text-white mt-10 mb-4 print:text-black' }, trimmed.replace('# ', '')));
...
const InputView = ({ onGenerate, initialData, isLoading, error }) => {
    const [formData, setFormData] = useState(initialData || { field: '', name: '', scope: '', location: '', budget: '', currency: 'USD', budgetType: 'Fixed', startDate: '', finishDate: '', duration: '' });
    const predefinedFields = [ "Construction & Engineering", "Software Development & IT", "Marketing & Advertising", "Healthcare & Pharmaceuticals", "Manufacturing & Logistics", "Education & Training", "Finance & Banking", "Energy, Oil & Gas", "Government & Public Sector", "Event Management", "Real Estate", "Telecommunications" ];
    const predefinedLocations = [ "Saudi Arabia", "United Arab Emirates", "Egypt", "Qatar", "Kuwait", "Bahrain", "Oman", "United States", "United Kingdom", "Canada", "Germany", "France", "Global / Remote" ];
    const locationCurrencyMap = { "Saudi Arabia": "SAR", "United Arab Emirates": "AED", "Egypt": "EGP", "Qatar": "QAR", "Kuwait": "KWD", "Bahrain": "BHD", "Oman": "OMR", "United States": "USD", "United Kingdom": "GBP", "Canada": "CAD", "Germany": "EUR", "France": "EUR", "Global / Remote": "USD" };
    const currencies = ["USD", "EUR", "GBP", "SAR", "AED", "EGP", "KWD", "QAR", "OMR", "BHD", "INR", "CNY", "JPY"];
    useEffect(() => { if (formData.location && locationCurrencyMap[formData.location]) setFormData(prev => ({ ...prev, currency: locationCurrencyMap[formData.location] })); }, [formData.location]);
    useEffect(() => {
        if (formData.startDate && formData.finishDate) {
            const start = new Date(formData.startDate); const end = new Date(formData.finishDate);
            if (end > start) {
    ...
    return React.createElement('div', { className: 'max-w-3xl mx-auto animate-fade-in-up' },
        ...
        React.createElement('div', { className: 'bg-dark-card-solid p-6 rounded-xl border border-dark-border space-y-4' },
            React.createElement('div', null,
                ...
                React.createElement('input', { name: 'field', list: 'field-options', value: formData.field, onChange: handleChange, placeholder: "Select or type field of work...", className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none' }),
                React.createElement('datalist', { id: 'field-options' }, predefinedFields.map((f, i) => React.createElement('option', { key: i, value: f })))
            ),
            React.createElement('div', null,
                React.createElement('label', { className: 'block text-sm font-medium text-brand-text-light mb-1' }, "Project Name"),
                React.createElement('input', { name: 'name', value: formData.name, onChange: handleChange, placeholder: "e.g., Riyadh Metro Line 7 Extension", className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none' })
            ),
            React.createElement('div', null,
                React.createElement('label', { className: 'block text-sm font-medium text-brand-text-light mb-1' }, "Scope Summary"),
                React.createElement('textarea', { name: 'scope', value: formData.scope, onChange: handleChange, placeholder: "Briefly describe the project deliverables, key objectives, and boundaries...", rows: 3, className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white resize-none focus:ring-2 focus:ring-brand-purple focus:outline-none' })
            ),
            React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-2 gap-4' },
                React.createElement('div', null,
                    ...
                    React.createElement('input', { name: 'location', list: 'location-options', value: formData.location, onChange: handleChange, placeholder: "Select or type location...", className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none' }),
                    React.createElement('datalist', { id: 'location-options' }, predefinedLocations.map((l, i) => React.createElement('option', { key: i, value: l })))
                ),
                 React.createElement('div', null,
                    React.createElement('label', { className: 'block text-sm font-medium text-brand-text-light mb-1' }, "Duration (Months)"),
                    React.createElement('input', { name: 'duration', type: 'number', min: '1', value: formData.duration, onChange: handleChange, placeholder: "e.g., 18", className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none' })
                )
            ),
            React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-2 gap-4' },
                React.createElement('div', null,
                    React.createElement('label', { className: 'block text-sm font-medium text-brand-text-light mb-1' }, "Project Start Date"),
                    React.createElement('input', { type: 'date', name: 'startDate', value: formData.startDate, onChange: handleChange, className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none placeholder-slate-500' })
                ),
                React.createElement('div', null,
                    React.createElement('label', { className: 'block text-sm font-medium text-brand-text-light mb-1' }, "Project Finish Date"),
                    React.createElement('input', { type: 'date', name: 'finishDate', value: formData.finishDate, onChange: handleChange, min: formData.startDate, className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none placeholder-slate-500' })
                )
            ),
            React.createElement('div', null,
                React.createElement('label', { className: 'block text-sm font-medium text-brand-text-light mb-1' }, "Total Budget"),
                React.createElement('div', { className: 'flex gap-2 mb-2' },
                    React.createElement('input', { name: 'budget', type: 'number', value: formData.budget, onChange: handleChange, placeholder: "e.g., 500000", className: 'w-full p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none' }),
                    React.createElement('select', { name: 'currency', value: formData.currency, onChange: handleChange, className: 'w-24 p-3 bg-dark-bg border border-dark-border rounded-lg text-white focus:ring-2 focus:ring-brand-purple focus:outline-none font-bold' }, currencies.map(c => React.createElement('option', { key: c, value: c }, c)))
                ),
                React.createElement('div', { className: 'flex gap-4' },
                    React.createElement('label', { className: 'flex items-center gap-2 cursor-pointer' },
                        React.createElement('input', { type: 'radio', name: 'budgetType', value: 'Fixed', checked: formData.budgetType === 'Fixed', onChange: handleChange, className: 'w-4 h-4 text-brand-purple bg-dark-bg border-dark-border focus:ring-brand-purple' }),
                        React.createElement('span', { className: 'text-sm text-brand-text-light' }, "Fixed Budget")
                    ),
                    React.createElement('label', { className: 'flex items-center gap-2 cursor-pointer' },
                        React.createElement('input', { type: 'radio', name: 'budgetType', value: 'Predicted', checked: formData.budgetType === 'Predicted', onChange: handleChange, className: 'w-4 h-4 text-brand-purple bg-dark-bg border-dark-border focus:ring-brand-purple' }),
            ...
            React.createElement('button', { onClick: () => onGenerate(formData), disabled: isLoading || !isFormValid, className: "w-full py-3 mt-2 font-semibold text-white bg-button-gradient rounded-lg shadow-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center" }, isLoading ? React.createElement(Spinner, { size: '6' }) : (initialData ? "Regenerate Plan" : "Generate Plan"))
        )
    );
};
...
const ComprehensivePlanView = ({ language, projectData, onUpdateProject, isLoading, setIsLoading, error, setError }) => {
    ...
    useEffect(() => { if (projectData.criteria) setCriteria(projectData.criteria); }, [projectData.criteria]);
    ...
    const handleReset = () => { onUpdateProject({ consultingPlan: null, criteria: null }); setCriteria(null); setError(null); setIsEditingCriteria(false); };
    const hasPlan = !!projectData.consultingPlan;
    const customControls = (
        React.createElement(React.Fragment, null,
            React.createElement('button', { onClick: () => { setError(null); setIsEditingCriteria(true); }, className: 'p-2 rounded-md text-brand-text-light hover:bg-white/10 hover:text-white transition-colors', title: "Edit Criteria" }, React.createElement(EditIcon, { className: "h-5 w-5" })),
            React.createElement('button', { onClick: handleReset, className: 'p-2 rounded-md text-brand-text-light hover:bg-white/10 hover:text-white transition-colors', title: "Refresh / New Plan" }, React.createElement(RefreshIcon, { className: "h-5 w-5" }))
        )
    );
    return React.createElement('div', { ref: fullscreenRef, className: "h-full flex flex-col text-white bg-dark-card printable-container" },
        hasPlan && !isEditingCriteria && React.createElement(FeatureToolbar, { title: t.dashboardConsultingPlan, containerRef: fullscreenRef, onZoomIn: handleZoomIn, onZoomOut: handleZoomOut, onToggleEdit: handleToggleEdit, isEditing: isEditing, onExport: handleExport, customControls: customControls }),
        React.createElement('div', { className: 'flex-grow min-h-0 overflow-auto' },
            React.createElement('div', {
               ref: contentRef, className: 'p-8 printable-content w-full pb-32 min-h-full',
               ...
               contentEditable: isEditing, suppressContentEditableWarning: true
                ...
                : (hasPlan && !isEditingCriteria)
                    ? React.createElement(ResultsView, { plan: projectData.consultingPlan, criteria: projectData.criteria || criteria })
                    : React.createElement('div', { className: 'h-full flex items-center justify-center' }, 
                        React.createElement(InputView, { onGenerate: handleGenerate, initialData: criteria, isLoading: false, error }))
            )
        )
    );
};
